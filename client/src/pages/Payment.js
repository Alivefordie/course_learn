...
import { Button, Modal, ProgressBar, Form, Row, Col, Container, Card } from "react-bootstrap";
...
const Payment = () => {
	...
	const generateQRcode = async () => {
		...
		const qrc = await QRCode.toDataURL(payload, { color: { dark: "#000", light: "#fff" } });
		setqrc(qrc);
		setLoading(false);
	};
									...
									<Card
										...
										<div className="body-col">
											<Card.Body
												...
												<Card.Text className="m-0">ระยะเวลา {item.attributes.duration}</Card.Text>
											</Card.Body>
										</div>
									</Card>
								))}
							</div>
							{data.length > 0 && <div className={styles.totalPrice}>TotalPrice: {totalPrice}</div>}
							...
							<Container className={styles.methodCon}>
								...
								{data.length === 0 ? (
									...
									<div className={styles.confirmBtn}>
										<Button onClick={handleConfirm} variant="dark" className={styles.confirmBtn}>
											Confirm
										</Button>
									</div>
								)}
